...
/** Nhãn biến thể: ưu tiên tên biến thể, nếu không thì ghép TOÀN BỘ thông số + màu. */
...
export function BienTheCard({ item }: { item: BienTheCardType }) {
  ...
  const { slug, bienTheId, tenSanPham, thuongHieu, tenBienThe, mauSac, trangThai, thongSoBienThe, anhChinh, gia, giaBan, phanTramGiam } = item;
  ...
  const hetHang = trangThai === 'HET_HANG';
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm transition hover:shadow-md">
      ...
      <Link href={href} className="relative block p-3">
        <ProductImage src={anhChinh} alt={tenSanPham} className={cn('aspect-square w-full rounded-lg', hetHang && 'opacity-50')} />
        {hetHang && (
          <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-md bg-sale/90 px-2.5 py-1 text-xs font-bold text-white">
            Sản phẩm đang hết
          </span>
        )}
      </Link>
      ...
      <div className="flex flex-1 flex-col px-3 pb-3">
        ...
        {thuongHieu && <p className="mt-0.5 text-xs font-medium text-gray-600">{thuongHieu}</p>}
        {nhanBt && <p className="mt-0.5 line-clamp-2 text-xs text-gray-500">{nhanBt}</p>}
        ...
        {hetHang ? (
          <div className="mt-3 rounded-lg bg-gray-100 py-2 text-center text-sm font-semibold text-gray-400">
            Hết hàng
          </div>
        ) : (
